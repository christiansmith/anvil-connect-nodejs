...
AccessToken.decode = function (token, publickey) {
  ...
  if (segments.length !== 3) {
    throw new Error('Invalid access token segments') // Error is caught in AccessToken.verify
  }
  ...
  return {
    ...
    header: JSON.parse(new Buffer(headerSeg, 'base64')),
    payload: JSON.parse(new Buffer(payloadSeg, 'base64')),
    signatureSeg: JSON.parse(new Buffer(signatureSeg, 'base64'))
  }
}
